...
date: "2020-12-26"
categories: [Statistical Learning Theory, Generalisation]
description: "A comprehensive, step-by-step mathematical proof of the classic PAC-Bayesian generalisation error bounds."
---
...
$$
where $\Pr(A)$ is the probability of event $A$, $\delta \in (0, 1]$ is the confidence parameter, and $r(m, \delta)$ -- a function of <em>sample size</em> $m$ and the confidence parameter $\delta$ -- is the <em>regularisation</em> that is satisfied:
$$
\lim_{m \to +\infty} r(m, \delta) = 0.
$$
PAC-Bayes upper generalisation bound is a kind of PAC learning. It was firstly proposed in 1999 @mcallester1999pac, and has attracted much of research interest. There has been many subsequent improvements made to tighten further this classic PAC-Bayes bound or to extend it to more general loss functions. However, the classic PAC-Bayes theorem is still the backbone. In this post, I will show how to prove this interesting theorem.
...
We then make a change of variable from $x$ to $\epsilon$ to utilise the given inequality in the assumption. Let's define:
$$
x = e^{2(m - 1) \epsilon^{2}}.
$$
