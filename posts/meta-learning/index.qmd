...
The difference between meta-learning and hyper-parameter optimisation is that the meta-parameter (also known as hyper-parameter) $\theta$ is shared across all tasks sampled from the task environment $p(\mathcal{D}, f)$ as highlighted in <span style="color: red;">red</span> colour in @eq-meta_learning_bilevel_optimisation.
